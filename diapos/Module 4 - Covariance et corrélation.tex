...
\begin{frame}[t]
 ...
 Quand les variables formant un vecteur aléatoire évoluent de manière indépendante, on peut utiliser la théorie précédente pour calculer  probabilités, moyennes, variances, etc.
...
\uncover<3->{\alert{Attention:} $A\perp B \rightarrow Cov(X,Y)=0$, mais $Cov(X,Y) \notrightarrow A\perp B$}
}
...
{\footnotesize $X$ prend de fortes valeurs quand $Y$ en prend des faibles par rapport à leur moyenne et vice versa}&
