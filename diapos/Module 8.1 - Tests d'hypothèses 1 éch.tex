...
\item Plus $\alpha$ est petit, plus $\beta$ est grand...\\donc on fixe $\alpha$ et on contrôle souvent $\beta$ avec la taille d'échantillon.
\end{itemize}
...
Si nous connaissons la loi de $\overline X$ sous $H_0$, nous sommes capables d'en déduire la valeur de $k$ qui nous donne la région critique désirée.
\end{frame}
...
\subsection{Hypothèse alternative unilatérale à droite}
...
\begin{enumerate}
  ...
  \item $\alpha^*\approx 0,33~\%$, alors que $P(H_0\text{\ est vraie}\;|\;X=4)\approx12,98\%$
